...
export default function Paginado({ page, setPage, maxTotal }) {
  const nextPage = () => {
  ...
  const onKeyInput = (e) => {
    if (e.keyCode === 13) {
      ...
      if (
        ...
        parseInt(e.target.value) > Math.ceil(maxTotal) ||
  ...
  return (
    <div className={styles.container}>
      ...
      <input
        ...
        value={page}
      />
      <p>/ {Math.ceil(maxTotal)}</p>
      <button disabled={page >= Math.ceil(maxTotal)} onClick={nextPage}>
        <svg
          width="20"
          height="27"
          viewBox="0 0 20 27"
